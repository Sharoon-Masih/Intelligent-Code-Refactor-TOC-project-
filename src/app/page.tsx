'use client'

import CodeEditor from "@/components/CodeEditor";
import ResultBox from "@/components/ResultBox";
import { useState } from "react";
import { motion } from "framer-motion"
import { Issue } from "@/utils/dfa";

interface analysisResponse {
  issues: Issue[],
  token: string[]
}
export default function Home() {
  const [code, setCode] = useState("// Write your code here");
  const [clicked, setClicked] = useState(false);
  const [res, setRes] = useState<analysisResponse | undefined>()

  async function onClickHandler(code: string) {
    const res = await fetch(`/api/analyze`, {
      method: "POST",
      body: JSON.stringify({ code }),
      headers: {
        "Content-Type": "application/json"
      }
    });
    if (res) {
      setClicked(false)
    }
    const data: analysisResponse = await res.json()
    return data
  }
  return (
    <main className="grid max-w-full w-full grid-cols-1 md:grid-cols-2 md:h-screen gap-2 bg-gradient-to-r from-red-600 via-purple-900 to-blue-800 p-5 overflow-hidden">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
        className="rounded-xl bg-[#0d1117b1] text-white p-3 shadow-xl">
        <CodeEditor code={code} setCode={setCode} />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, x: 50 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.8, ease: "easeOut" }}
        className="flex flex-col w-full overflow-y-auto bg-white/10 backdrop-blur-md rounded-xl shadow-xl overflow-hidden border border-white/20"
      >
        <div className="flex items-center justify-between p-4 bg-black bg-opacity-70 border-b border-white/20 sticky top-0 z-10">
          <h2 className="text-white font-semibold text-lg">Output Panel</h2>
          <button
            disabled={clicked}
            onClick={async () => {
              setClicked(true);
              setRes(await onClickHandler(code));
            }}
            className="relative inline-flex items-center justify-center px-6 py-2 overflow-hidden text-sm font-medium text-white bg-gradient-to-br from-purple-600 to-blue-500 rounded-xl hover:from-purple-700 hover:to-blue-600 focus:ring-2 focus:ring-blue-300 disabled:opacity-50"
          >
            {clicked ?
              <div role="status">
                <svg aria-hidden="true" className="w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                  <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                </svg>
                <span className="sr-only">Loading...</span>
              </div>
              : "Analyze"}
          </button>
        </div>
        <motion.div
          key={JSON.stringify(res?.issues)}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="flex-grow bg-neutral-200 flex flex-col">
          <ResultBox Output={res?.issues} />
        </motion.div>
      </motion.div>
    </main>
  );
}
